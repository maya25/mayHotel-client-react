...
import { getDateRange, generateNormalizedArray, getDateRangeByDate } from "../../../utils/helpers";
...
const AllEvents = () => {
  ...
  const [userAllowedDates, setUserAllowedDates] = useState(getDateRange(new Date(), 10));
  ...
  const filteredDates = allowedDates.filter((date) =>
    userAllowedDates.some(elem => {
      return JSON.stringify(date) === JSON.stringify(elem);
    }))
